# Papers

This page summarizes the publications of the Gyro project.

## Gyro Logic

### Jxiv (English)

- DOI: [10.51094/jxiv.4159](https://doi.org/10.51094/jxiv.4159)
- Repository: https://github.com/gitGyro-Dev/gyrologic

### Jxiv (Japanese)

- DOI: [10.51094/jxiv.4597](https://doi.org/10.51094/jxiv.4597)
- Repository: https://github.com/gitGyro-Dev/gyrologic

### Zenodo

- Version: v2.4
- DOI: [10.5281/zenodo.19674375](https://doi.org/10.5281/zenodo.19674375)

### Minimal Formal Model v1 — Jxiv (English)

**A Minimal Formal Model for Gyro Logic: Local Articulation, Stability Scenes, and Contextual Tracing**

- Platform: Jxiv
- Status: Published
- DOI: [10.51094/jxiv.5641](https://doi.org/10.51094/jxiv.5641)
- Repository: https://github.com/gitGyro-Dev/gyrologic
- External critical review: Completed 2026-08-10 (Claude, Gemini) — see `gyrologic/project_cycle/2026-08-10_external_critical_review_followup.md`

## GyroOS

Status: Design and implementation in progress.

## GyroAuth

Repository: https://github.com/gitGyro-Dev/gyroauth

### Primary Paper — English

**GyroAuth: Authentication as Stability-Based Selection over State Convergence**

- Platform: Jxiv
- Status: Published
- Role: Primary manuscript
- DOI: [10.51094/jxiv.4600](https://doi.org/10.51094/jxiv.4600)

### Primary Paper — Japanese

**GyroAuth：状態収束に対する安定性に基づく認証**

- Platform: Jxiv
- Status: Published
- Role: Japanese translation
- DOI: [10.51094/jxiv.5341](https://doi.org/10.51094/jxiv.5341)

### Applied Paper — English

**Trajectory-Based Vulnerability Response: A GyroAuth Applied Model for Post-Login Security Monitoring**

- Platform: Jxiv
- Status: Published
- DOI: [10.51094/jxiv.5416](https://doi.org/10.51094/jxiv.5416)
- Artifact path: `examples/trajectory_vulnerability_response/`

### Applied Paper — Japanese

**Trajectoryに基づく脆弱性対応：ログイン後セキュリティ監視のためのGyroAuth応用モデル**

- Platform: Jxiv
- Status: Published
- DOI: [10.51094/jxiv.5440](https://doi.org/10.51094/jxiv.5440)
- Artifact path: `examples/trajectory_vulnerability_response/`

## Citation

Please cite the corresponding DOI when referring to each publication.
